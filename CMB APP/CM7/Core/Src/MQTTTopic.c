...
void publish_channel_config(mqtt_client_t *client, int slot) {
    char topic[128];
    snprintf(topic, sizeof(topic), "CMM/slot%d/config/channels_info", slot);
...
void handle_incoming_topic_data(const char *topic, const char *data, size_t len) {
    ...
    snprintf(topic_buffer, sizeof(topic_buffer), "CMM/slot%d/status/ethernet/link", slot);
    if (strcmp(topic, topic_buffer) == 0) {
...
//			LWIP_PLATFORM_DIAG(("Failed to parse ACPWR topic: %s\n", topic));
	...
	// Existing CMM topic handling continues here...
...
void mqtt_subscribe_to_topics(mqtt_client_t *client, const struct mqtt_connect_client_info_t* client_info, int slot) {
    ...
    // List of all the CMM/slot%d/... topics to subscribe to
    const char* cmm_topics[] = {
        "control/iap",
    ...
    // Subscribe to each CMM topic
    for (size_t i = 0; i < sizeof(cmm_topics) / sizeof(cmm_topics[0]); i++) {
        snprintf(topic, sizeof(topic), "CMM/slot%d/%s", slot, cmm_topics[i]);
