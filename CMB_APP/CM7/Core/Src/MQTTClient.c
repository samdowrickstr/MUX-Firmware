...
#if LWIP_IPV4
#define LWIP_MQTT_EXAMPLE_IPADDR_INIT = IPADDR4_INIT(PP_HTONL(LWIP_MAKEU32(192, 168, 35, 8)))
#else
...
//    LWIP_PLATFORM_DIAG(("MQTT client \"%s\" connection cb: status %d\n", client_info->client_id, (int)status));
    ...
    if (status == MQTT_CONNECT_ACCEPTED) {
        ...
        char topic[128];
        snprintf(topic, sizeof(topic), "CMB/slot%d/status", slot); // Use the slot number in the status topic
...
//        LWIP_PLATFORM_DIAG(("Failed to create MQTT client\n"));
    ...
    snprintf(mqtt_client_id, sizeof(mqtt_client_id), "CMB%d", slot);
    snprintf(mqtt_will_topic, sizeof(mqtt_will_topic), "CMB/slot%d/status", slot);
	osDelay(200);
